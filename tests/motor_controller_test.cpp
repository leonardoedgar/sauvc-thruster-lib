...
TEST_F(MotorControllerTest, move_forward_with_desired_motors_and_speed) {
    ...
    int test_id {1};
    std::vector<int> motor_ids_to_run = sample_controller.motor_ids_for_motion["forward"];
    std::string desired_output = "";
    for (int motor_id_to_run: motor_ids_to_run) {
        desired_output += "Motor with id: " + std::to_string(motor_id_to_run) + " is running at speed: " +
